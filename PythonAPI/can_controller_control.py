...
class AutopilotCAN(object):
    def __init__(self):
        ...
        self.speed = 0
        self.msg_buffer = queue.Queue(maxsize=10)
    
    def can_sniffer(self):
    ...
    def can_throttle_controller(self):        
        ...
        #receive_message = self.msg_buffer.get()
        self.msg_buffer.put(self.receive_message)
        #print(self.msg_buffer.queue)
        msg_in_buffer = self.msg_buffer.get()

        if msg_in_buffer == None:
            return 
        elif(msg_in_buffer.arbitration_id == 0x1f5):  # Throttle
            decode_msg = self.db.decode_message(msg_in_buffer.arbitration_id, msg_in_buffer.data)
            #print("throttle: ",float(decode_msg['CTRL_THROTTLE']))
    ...
    def can_steer_controller(self):                          
        ...
        if msg_in_buffer == None:
            return 
        if(msg_in_buffer.arbitration_id == 0x14A):  # Steer
            decode_msg = self.db.decode_message(msg_in_buffer.arbitration_id, msg_in_buffer.data)
            #print(msg_in_buffer)
            #print("steer angle: ",decode_msg['STEER_ANGLE']/500)
        ...
        #     decode_msg = self.db.decode_message(self.receive_message.arbitration_id, self.receive_message.data)
        #     #print(self.receive_message.data)
        #     #print("steer angle: ",decode_msg['STEER_ANGLE']/500)
        #     return decode_msg['STEER_ANGLE']/500
    ...
    def can_brake_controller(self):                         
        ...
        if msg_in_buffer == None:
            return 
        if(msg_in_buffer.arbitration_id == 0x1E7):  # Brake 
            decode_msg = self.db.decode_message(msg_in_buffer.arbitration_id, msg_in_buffer.data)
            #print("brake: ",decode_msg['BRAKE_PRESSURE1'])
